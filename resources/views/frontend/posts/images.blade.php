	...
	<div class="mb-5" style="position:relative;">
		<ul style="padding:0px;margin:0px;padding-left:-3px;padding-right:-3px;">
			@foreach($post->postFiles as $k=>$postfile)
				...
				<?php break; ?>
				@endif
				<li style="list-style:none;float:left;padding:3px;margin:0px;"><a rel="{!! $k !!}" href="javascript:void(0);" onclick='showPostFilesModal(<?php echo json_encode($htmlImage); ?>,{!! $k !!});'>{!! HTML::image('public/upload/posts/'.$post->postUser->uniq_username.'/'.$postfile->image_name, "", array("style"=>"height:150px;border-radius:10px;", "class"=>"img-fluid border-radius")) !!}</a></li>
			@endforeach
		</ul>
