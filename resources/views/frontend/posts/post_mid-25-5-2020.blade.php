...
@foreach($posts as $post)
	...
	<div class="card mb-4" id="postcard{!! base64_encode($post->id) !!}">
		<div class="card-body">
			...
			@if($is_allowed_sub)
				<p><?php
					preg_match_all('/<div class=\"giphyimgareacreatepost\">(.*?)<\/div>/s',  $post->description, $out);
					preg_match('~src="(.*)"\s*itemprop="image"[^>]*>~', $post->description, $out);
					//$pattern = "/<p[^>]*><br><\\/p[^>]*>/"; // regular expression
					$pattern = "/(<p><br>)+(<\/p>)/i"; // regular expression
					...
					$post->description  =  preg_replace($pattern, '', $post->description); ?>
					{!! $post->description !!}
				</p>
