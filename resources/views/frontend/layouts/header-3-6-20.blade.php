									...
									@foreach($notificationsG as $notify)
										<div class="d-flex flex-row mb-3 pb-3 border-bottom">
											...
											<div class="pl-3">
												...
												@if(in_array($notify->type, [1,2,3,7])) <?php 
												...
												@elseif(in_array($notify->type, [16]))
													<?php $notifydetailurl = url($route_prefix.'/'.$notify->user_id);
													$streameclass = "steamerst_link";  ?>
												@elseif(in_array($notify->type, [12])) <?php 
													$Ndaata  = json_decode($notify->data, true);
												...
												@endif
												<a href="{!! $notifydetailurl !!}" onclick="markNotificationRead({!! $notify->id !!});" class="{!! $streameclass !!}">
													<p class="{!! ($notify->status == 1)?'font-weight-medium':'font-weight-bold notified notified'.$notify->id !!} mb-1">{!! $notify->message !!}</p>
				...
				@if(Auth::check())
					...
					@if(Auth::check())
						<div class="dropdown-menu dropdown-menu-right mt-3">
							<a class="dropdown-item steamerst_link" href="{!! url($route_prefix.'/'.Auth::User()->uniq_username) !!}">{!! getLabels('my_profile') !!}</a>
