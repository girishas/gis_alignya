@if(Auth::check())
	...
	<div class="modal fade bd-example-modal-lg" id="postLiveModal" tabindex="-1" role="dialog" aria-modal="true">
		<div class="modal-dialog modal-lg">
			{!! Form::open(array('url' => array($route_prefix.'/'.Auth::User()->uniq_username.'/'.env('POSTS_SUFFIX')), 'class' =>'steamerstudio_form needs-validation tooltip-label-right', 'files' => true, 'id'=>'post_form', 'name'=>'Search')) !!}
				<div class="modal-content">
					<div class="modal-header">
					...
					<div class="modal-body bg_input">
						...
						<div class="form-row">
							{!! Form::hidden('who_can_see', 1, array('class' => 'who_can_see_input')) !!}
							{!! Form::hidden('subscription_level', 0, array('class' => 'subscription_level_input')) !!}
							@if(Auth::User()->is_complete_profile)
								...
								<label class="form-group has-float-label mb-6 position-relative error-l-40 col-md-6">
									{!! Form::select('who_can_see_sub_level', ['1'=>'public', '2' => 'Followers', '3'=>'Only Me', '4'=>'Level 1', '5'=>'Level 2', '6'=>'Level 3'], null, array('class' => 'who_can_see_sub_level input-lg form-control'))!!}
							...
							@else
								...
								<label class="form-group has-float-label mb-6 position-relative error-l-40 col-md-6">
									{!! Form::select('who_can_see_sub_level', ['1'=>getLabels('public'), '2' => getLabels('followers'), '3'=>getLabels('only_me')], null, array('class' => 'who_can_see_sub_level input-lg form-control'))!!}
									<span>{!! getLabels('Visible_to') !!}</span>
								</label>
							@endif
						</div>
	...
	<div class="modal fade" id="forceLoginModal" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-dismiss="modal">{!! getLabels('Close') !!}</button>
						<a href="{!! url($route_prefix, 'login') !!}" class="btn btn-primary steamerst_link" title="Login">{!! getLabels('login') !!}</a>
					</div>
				</div>
			</div>
		</div>