...
@section('content')
  <main>
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <h1>{!! getLabels('Page_Images') !!}</h1>
					<div class="text-zero top-right-button-container">
						<a href="{!! url($route_prefix.'/page_images/add') !!}" class="steamerst_link btn btn-primary btn-lg top-right-button mr-1">{!! getLabels('add_new_page_images') !!}</a>
                    </div>
                    <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                        <ol class="breadcrumb pt-0">
                            <li class="breadcrumb-item">
							  ...
							  <li class="breadcrumb-item active" aria-current="page">{!! getLabels('Page_Images') !!}</li>
                        </ol>
                    </nav>
                ...
                <div class="col-12">
                    <div class="card mb-4">
                        <div class="card-body">
                            ...
                            {!! Form::open(array('url' => array($route_prefix.'/page_images'), 'class' =>'steamerstudio_searchform', 'name'=>'Search')) !!}
								<div class="form-body">
									<div class="row">
										<div class="col-lg-6">
										...
										<div class="col-lg-3">               
											...
											<a class="btn btn-dark mb-1 steamerst_link" href="{!! url($route_prefix, 'page_images') !!}">{!! getLabels('show_all') !!}</a>
										</div>
									</div>
								</div>
							{!!Form::close()!!}
                            ...
                            <table class="table">
                                <thead class="thead-light">
                                    <tr>
										...
										<th> {!! SortableTrait::link_to_sorting_action('page_title', getLabels('Title')) !!} </th>
										<th> {!! SortableTrait::link_to_sorting_action('height', getLabels('height')) !!} </th>
										<th> {!! SortableTrait::link_to_sorting_action('width', getLabels('width')) !!} </th>
                                    ...
                                    @if(!$data->isEmpty())
										@foreach($data as $val)
											<tr class="odd gradeX"><?php
												...
												<td>
													@if($val->type == 1)
														{!! showImage($val->image, "img-thumbnail", "","", $val->page_title, 'page_images',"max-width:85px;") !!}
													@else
													{!! getLabels('video') !!}
													@endif
												</td>
												...
												<td class="text-center">
													@if($val->status == 1)
														<a class="steamerst_status" href="{!! url($route_prefix.'/page_images/status/'.$val->id) !!}" title="{!! getLabels('decline') !!}"><span class="badge badge-pill badge-secondary "> {!! config('constants.STATUS.1')!!} </span></a>
													@else
														<a class="steamerst_status" href="{!! url($route_prefix.'/page_images/status/'.$val->id) !!}" title="{!! getLabels('approve') !!}"><span class="badge badge-pill badge-danger "> {!! config('constants.STATUS.0')!!} </span></a>
													@endif
												</td>
												<td>
													<div class="btn-group float-none-xs">
														...
														<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 25px, 0px);">
															<a class="steamerst_link dropdown-item" href="{!! url($route_prefix.'/page_images/edit/'.$val->id) !!}">{!! getLabels('edit') !!}</a>
