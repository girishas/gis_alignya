<script>
  $(document).ready(function(){
    ...
    if(localStorage.getItem('popup_name') && localStorage.getItem('popup_name').indexOf('objective') !== -1){
       viewobjective(JSON.parse(localStorage.getItem('popup_id'))[0]);   

    }
    if(localStorage.getItem('popup_name') && localStorage.getItem('popup_name').indexOf('measure') !== -1){
      viewMeasure(JSON.parse(localStorage.getItem('popup_id'))[1]);
    }
    if(localStorage.getItem('popup_name') && localStorage.getItem('popup_name').indexOf('initiative') !== -1){
      view_initiativepop(JSON.parse(localStorage.getItem('popup_id'))[1]);
    }
  $("#viewinitiativemodalhide").click(function(){
    if(window.location.pathname.indexOf('initiatives') !== -1 ){
      $("#viewinitiativemodal").modal('hide');
    }else{
      popup_name = JSON.parse(localStorage.getItem('popup_name'));
      popup_name.pop();
      localStorage.setItem('popup_name',JSON.stringify(popup_name));
      idarr = JSON.parse(localStorage.getItem('popup_id'));
      idarr.pop();
      localStorage.setItem('popup_id',JSON.stringify(idarr));
    }
  });
  ...
  $("#view_measuremodal_hide").click(function(){
    if(window.location.pathname.indexOf('measures') !== -1 ){
    }else{
      popup_name = JSON.parse(localStorage.getItem('popup_name'));
      popup_name.pop();
      localStorage.setItem('popup_name',JSON.stringify(popup_name));
...
function viewMeasure(id){
    if(window.location.pathname.indexOf('objectives') !== -1){
      var popup_name = [];
      popup_name = JSON.parse(localStorage.getItem('popup_name')) || [];
      if(localStorage.getItem('popup_name').indexOf('measure') !== -1){

      }else{
        popup_name.push('measure');
      }
      localStorage.setItem('popup_name', JSON.stringify(popup_name));
    }else{
      localStorage.setItem('popup_name','');
    }
    $(".is_popup").val(1);
    var idarr = [];
    idarr = JSON.parse(localStorage.getItem('popup_id')) || [];
...
function updateMeasure(id){
    ...
    function view_initiativepop(id){
        if(window.location.pathname.indexOf('objectives') !== -1){
          var popup_name = [];
          popup_name = JSON.parse(localStorage.getItem('popup_name')) || [];
          if(localStorage.getItem('popup_name').indexOf('initiative') !== -1){
          }else{
            popup_name.push('initiative');
          }
          localStorage.setItem('popup_name', JSON.stringify(popup_name));
        }else{
          localStorage.setItem('popup_name','');         
