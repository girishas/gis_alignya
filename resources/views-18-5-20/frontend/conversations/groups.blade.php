...
@section('content')
	...
	<main>
					...
					<div class="d-flex flex-row justify-content-between mb-3 chat-heading-container">
						<div class="d-flex flex-row chat-heading">
							<a class="d-flex headingimage" href="javascript:void(0);">
								@if($curgroup and $curgroup->icon and file_exists('public/upload/groups/'.$curgroup->icon))
									{!! HTML::image('public/upload/groups/'.$curgroup->icon, $curgroup->name, array("class"=>"img-thumbnail border-0 rounded-circle ml-0 mr-4 list-thumbnail align-self-center small")) !!}
								@endif
							</a>
								...
								<div
									...
									<div class="min-width-zero">
										<a href="javascript:void(0);">
											<p class="list-item-heading mb-1 truncate headingname">{!! $curgroup?$curgroup->name:"" !!}</p>
										</a>
										 <p class="mb-1 text-muted text-small maingm">@if($curgroup) {!! $curgroup->totalMember !!} {!! $curgroup->totalMember > 1?getLabels('Members'):str_singular(getLabels('Members')) !!} @endif</p>
        ...
        <div class="app-menu">
            <ul class="nav nav-tabs card-header-tabs ml-0 mr-0 mb-1" role="tablist">
                <li class="nav-item w-50 text-center">
                    <a class="nav-link  steamerst_link" id="first-tab" href="{!! url($route_prefix.'/messages') !!}">{!! getLabels('messages') !!}</a>
                </li>
                <li class="nav-item w-50 text-center">
                    <a class="nav-link active steamerst_link" id="second-tab" href="{!! url($route_prefix.'/groups') !!}">{!! getLabels('Groups') !!}</a>
                </li>
            </ul>
								...
								@foreach($my_grps as $my_grp)
									<?php $group_json[$my_grp->slug]  = $my_grp;
										...
										//pr($group_json_members); die;
											 ?>
									<div class="d-flex flex-row mb-1 border-bottom pb-3 mb-3 chat_list_users {!! (!empty($curgroup->slug) and $my_grp->slug == $curgroup->slug)?'active_chat':'' !!}" data-username="{!! $my_grp->slug !!}" style="cursor:pointer;" onclick="onUserSelected(this);">
		...
		<form onsubmit="return sendMessage();" id="form-send-message">
			...
			<div class="chat-input-container d-flex justify-content-between align-items-center">
				...
				<div class="chat_text_area" style="" contenteditable="true"> </div>
	...
	<script type="text/javascript">
		...
		var chat_type = "group";
		var myName = "{!! Auth()->User()->uniq_username !!}";
		var myFullname = "{!! Auth()->User()->first_name.' '.Auth()->User()->last_name !!}";
		var myImage = "{!! url('public/upload/users/profile-photo/'.Auth()->User()->photo) !!}";
	  ...
	  function enterName() {
		...
		function messageHTML(type, message, senderInfo, msgDateStr){
			...
			if(type == 'sent'){
				...
				var muserimage  = "{!! url('public/upload/users/profile-photo') !!}/"+senderInfo.photo;
			}
