...
class UserController extends Controller
{
    ...
    {
       Parent::__construct();
		$this->middleware('auth', ['except' => ['resizeProfileImageGoogle', 'resizeProfileImage', 'resizeProfileImageFacebook', 'verifyMail', 'findOrCreateUser', 'redirectToProvider', 'handleProviderCallback', 'admin_login', 'admin_register', 'login', 'register', 'forgot_password', 'resetpassword', 'admin_forgot_password']]);
    }
	...
	public function admin_add(){
		...
		$page_title = getLabels("add_new_user");
		...
		if($this->request->isMethod('post')){
			...
			if ( $validator->fails() ) {
				...
				if($user){
					...
					return response()->json(['type' => 'success', 'url'=> url(env('ADMIN_PREFIX'), 'users'), 'message' => getLabels('user_saved_successfully')]);
				}else{
	...
	public function admin_edit($id = null){ 
		...
		$page_title = getLabels("update_user");
		$countries  = Country::where('status', 1)->orderBy('name', 'asc')->pluck('name', 'id')->toArray();
		if($this->request->isMethod('post')){
	...
	public function profile($username = null){
		...
		return view('frontend.users.profile', compact('page_title', 'data', 'following', 'followers', 'posts', 'myfollowers'));
	}
