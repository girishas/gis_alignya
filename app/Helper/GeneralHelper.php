<?php
	...
	function isFollowing($userid){
		$following  = Follower::where('user_id', Auth::id())->where('to_user_id', $userid)->count();
		$follower   = Follower::where('to_user_id', Auth::id())->where('user_id', $userid)->count();
		$is_following = array();
		if($following > 0){
			$is_following['type'] = "following";
			$is_following['userDetail'] = DB::table('users')->where('status', 1)->where('id', $userid)->first();
		}elseif($follower > 0){
			$is_following['type'] = "follower";
			$is_following['userDetail'] = DB::table('users')->where('status', 1)->where('id', $userid)->first();
		}
		return $is_following;
	}
